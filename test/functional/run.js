...
function main() {
  ...
  if (flags['serve-deck-from']) {
    testRunner.launchStaticServer(flags['serve-deck-from'], 9000);
  }

  if (flags['fetch-fixtures-from']) {
    testRunner.fetchTestFixtures(flags['fetch-fixtures-from']);
  }
...
Run functional tests against Deck. Spins up a mock server to play back network
responses so you don't need to deploy lots of infrastructure and Spinnaker to
exercise Deck.
...
Options:
--serve-deck-from      Path to a compiled build of Deck.
...
--fetch-fixtures-from  GCS bucket to fetch network fixtures from if they're
                       missing on disk. Example: "gs://foo/captures".
--help                 Print this message.

Example usage:

# Run tests in headless Chrome with a prebuilt Deck and without
# Gate. Fetch network fixtures from gs://my-bucket/fixtures and save browser
# logs when tests complete.
test/functional/run.js --serve-deck-from build/webpack --replay-fixtures \\
    --browser chrome --headless --savelogs \\
    --fetch-fixtures-from gs://my-bucket/fixtures
`.trim(),
  );
  process.exit(0);
}
