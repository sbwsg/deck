...
const config = {
  ...
  beforeTest: function(test) {
    ...
    testRun = { file: test.file };
    if (!flags['replay-fixtures'] && !flags['record-fixtures']) {
      return;
    }
    testRun.fixtureFile = test.file + '.mountebank_fixture.json';
  ...
  afterTest: function(test) {
    if (flags['record-fixtures']) {
      if (test.passed) {
        mountebankService.saveRecording(testRun.fixtureFile).then(() => {
    ...
    if (flags.savelogs && browser.sessionId) {
      ...
      fs.writeFileSync(outPath, JSON.stringify(testRun.browserLogs, null, 4));
